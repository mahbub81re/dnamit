...
// কম্পোনেন্ট ডিক্লারেশনে প্রপস রিসিভ করুন
export default function StudentAdmissionForm({ initialData, onSuccess }: { initialData?: any, onSuccess?: () => void }) {
  const [loading, setLoading] = useState(false);
  
  // যদি initialData থাকে তবে সেটি বসবে, না থাকলে ডিফল্ট
  const [formData, setFormData] = useState(initialData || {
    name: '', roll: '', class: 'Class 1',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        alert(initialData ? "তথ্য আপডেট হয়েছে!" : "ভর্তি সফল হয়েছে!");
        if (onSuccess) onSuccess(); // ম্যানেজমেন্ট পেজ রিফ্রেশ করবে
      }
